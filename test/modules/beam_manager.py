...
from dax.modules.beam_manager import BeamManager, NO_BEAM, BeamAssignmentError
...
class GetBeam2TestCase(unittest.TestCase):
    ...
    def test_num_beams_bounds(self):
        ...
        for t in range(self.N):
            b = self.s.m.get_beam(t, True)
            self.assertNotEqual(b, NO_BEAM, 'Returned NO_BEAM while action was required')
        ...
        for t in range(self.N):
            b = self.s.m.get_beam(t, True)
            self.assertEqual(b, NO_BEAM, 'Did not return NO_BEAM while no action was required')
