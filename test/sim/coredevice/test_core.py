...
class CoreTestCase(unittest.TestCase):
    ...
    def test_constructor_signature(self):
        ...
        with self.assertRaises(TypeError, msg='Core class constructor did not match expected signature'):
            # Not adding _key
            dax.sim.coredevice.core.Core(dmgr=self.managers.device_mgr, ref_period=1e-9)
        with self.assertRaises(TypeError, msg='Core class constructor did not match expected signature'):
            # Not adding ref period
            # noinspection PyArgumentList
            dax.sim.coredevice.core.Core(dmgr=self.managers.device_mgr, _key='core')

        # Test with correct arguments
        self.assertIsNotNone(dax.sim.coredevice.core.Core(dmgr=self.managers.device_mgr, ref_period=1e-9, _key='core'))
